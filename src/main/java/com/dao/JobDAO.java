...
public List<Jobs> getJobsAndLocationAndCate (String category, String location) {
	...
	public int countJobs() {
		try (PreparedStatement ps = conn.prepareStatement("select count(*) from jobs"); ResultSet rs = ps.executeQuery()) {
	...
	public int countActiveJobs() {
		try (PreparedStatement ps = conn.prepareStatement("select count(*) from jobs where lower(status)='active'"); ResultSet rs = ps.executeQuery()) {
			return rs.next() ? rs.getInt(1) : 0;
		} catch (Exception e) {
			e.printStackTrace();
			return 0;
